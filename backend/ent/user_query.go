...
func (_q *UserQuery) QueryTags() *UserTagQuery {
...
func (_q *UserQuery) QueryUserTagAssignments() *UserTagAssignmentQuery {
...
func (_q *UserQuery) WithTags(opts ...func(*UserTagQuery)) *UserQuery {
	query := (&UserTagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithUserTagAssignments(opts ...func(*UserTagAssignmentQuery)) *UserQuery {
	query := (&UserTagAssignmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withTags; query != nil {
		if err := _q.loadTags(ctx, query, nodes,
			func(n *User) { n.Edges.Tags = []*UserTag{} },
			func(n *User, e *UserTag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withUserTagAssignments; query != nil {
		if err := _q.loadUserTagAssignments(ctx, query, nodes,
			func(n *User) { n.Edges.UserTagAssignments = []*UserTagAssignment{} },
			func(n *User, e *UserTagAssignment) {
...
func (_q *UserQuery) loadTags(ctx context.Context, query *UserTagQuery, nodes []*User, init func(*User), assign func(*User, *UserTag)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*User)
	nids := make(map[int64]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(usertag.FieldID), joinT.C(user.TagsPrimaryKey[1]))
...
func (_q *UserQuery) loadUserTagAssignments(ctx context.Context, query *UserTagAssignmentQuery, nodes []*User, init func(*User), assign func(*User, *UserTagAssignment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usertagassignment.FieldUserID)
	}
	query.Where(predicate.UserTagAssignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserTagAssignmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
