...
func validatePlanRequiredForTest(name string, groupID int64, price float64, validityDays int, validityUnit string, originalPrice *float64) error {
	return validatePlanRequired(name, groupID, price, validityDays, validityUnit, originalPrice, 0, 0, 0, false, nil)
}

func TestValidatePlanRequired_AllValid(t *testing.T) {
	err := validatePlanRequiredForTest("Pro", 1, 9.99, 30, "days", nil)
	require.NoError(t, err)
}

func TestValidatePlanRequired_EmptyName(t *testing.T) {
	err := validatePlanRequiredForTest("", 1, 9.99, 30, "days", nil)
	require.Error(t, err)
...
func TestValidatePlanRequired_ZeroGroupID(t *testing.T) {
	err := validatePlanRequiredForTest("Pro", 0, 9.99, 30, "days", nil)
	require.Error(t, err)
...
func TestValidatePlanRequired_NameValidatedFirst(t *testing.T) {
	err := validatePlanRequiredForTest("", 0, 0, 0, "", nil)
	require.Error(t, err)
	require.Contains(t, err.Error(), "plan name")
}
...
func TestValidatePlanRequired_NegativeOriginalPrice(t *testing.T) {
	neg := -10.0
	err := validatePlanRequiredForTest("Pro", 1, 9.99, 30, "days", &neg)
	require.Error(t, err)
...
func TestValidatePlanRequired_ZeroOriginalPrice(t *testing.T) {
	zero := 0.0
	err := validatePlanRequiredForTest("Pro", 1, 9.99, 30, "days", &zero)
	require.NoError(t, err)
}

func TestValidatePlanRequired_ValidOriginalPrice(t *testing.T) {
	op := 19.99
	err := validatePlanRequiredForTest("Pro", 1, 9.99, 30, "days", &op)
	require.NoError(t, err)
}
