...
func TestOpenAIGatewayServiceLegacyLoadAwarenessHighestSchedulingOverridesLowRatePriority(t *testing.T) {
	now := time.Now()
	cheap := upstreamCostTestAccount(61, UpstreamBillingProbeStatusOK, 0.01, now.Add(-time.Minute), 30*time.Minute)
	cheap.Status, cheap.Schedulable, cheap.Concurrency, cheap.Priority = StatusActive, true, 1, 0
	highest := upstreamCostTestAccount(62, UpstreamBillingProbeStatusOK, 0.8, now.Add(-time.Minute), 30*time.Minute)
	highest.Status, highest.Schedulable, highest.Concurrency, highest.Priority = StatusActive, true, 1, 50
	highest.Extra[AccountExtraHighestSchedulingMode] = true
	accounts := []Account{*cheap, *highest}
	groupID := int64(1)
	...
	tests := []struct {
		...
		{name: "load-aware candidates"},
		{name: "load lookup failure", loadErr: errors.New("load unavailable")},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := &OpenAIGatewayService{
				...
				concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{
					loadBatchErr: tt.loadErr,
					loadMap: map[int64]*AccountLoadInfo{
						cheap.ID:   {AccountID: cheap.ID, LoadRate: 0},
						highest.ID: {AccountID: highest.ID, LoadRate: 90},
					},
					acquiredIDs: &acquiredIDs,
				}),
			}

			selection, err := svc.SelectAccountWithLoadAwareness(context.Background(), &groupID, "", "gpt-test", nil)
			require.NoError(t, err)
			require.Equal(t, highest.ID, selection.Account.ID)
			require.Equal(t, []int64{highest.ID}, acquiredIDs)
			if selection.ReleaseFunc != nil {
				selection.ReleaseFunc()
			}
		})
