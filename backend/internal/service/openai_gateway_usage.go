...
func (s *OpenAIGatewayService) BuildOpenAIImageCostSummary(ctx context.Context, input *OpenAIRecordUsageInput) *OpenAIImageCostSummary {
	...
	longContextBillingGate := openAILongContextBillingGate(input.Account)
	cost, err := s.calculateOpenAIRecordUsageCost(ctx, result, apiKey, billingModels, multiplier, imageMultiplier, videoMultiplier, baseMultiplier, tokens, serviceTier, longContextBillingGate)
	if err != nil || cost == nil {
		return summary
	}
