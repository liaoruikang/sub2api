...
func (s *OpenAIGatewayService) BuildOpenAIImageCostSummary(ctx context.Context, input *OpenAIRecordUsageInput) *OpenAIImageCostSummary {
	...
	if user != nil && apiKey.GroupID != nil && apiKey.Group != nil {
		...
		multiplier = apiKey.Group.BillingRateMultiplierForBaseAt(multiplier, now)
	}
	baseMultiplier := multiplier
	multiplier, imageMultiplier := computePeakAwareMultipliers(apiKey, baseMultiplier, now)
	...
	longContextBillingEnabled := input.Account != nil && input.Account.IsOpenAILongContextBillingEnabled()
	cost, err := s.calculateOpenAIRecordUsageCost(ctx, result, apiKey, billingModels, multiplier, imageMultiplier, videoMultiplier, baseMultiplier, tokens, serviceTier, longContextBillingEnabled)
	if err != nil || cost == nil {
		return summary
	}
