...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	canonicalCount := len(schedulerCanonicalBuckets(groupID))
	...
	require.Equal(t, 4*len(schedulerSnapshotPlatforms()), accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.Equal(t, len(schedulerCanonicalBuckets(groupID)), reopenCount)
	...
	require.Equal(t, 2*len(schedulerSnapshotPlatforms())+5, accounts.callCount())
	require.Equal(t, len(schedulerSnapshotPlatforms())+3, accounts.groupCallCount(groupID))
...
func TestSchedulerFullRebuildPreservesGroupZeroActiveHistoricalAndInvalidRegistryBuckets(t *testing.T) {
	...
	require.NoError(t, svc.rebuildFullSnapshot(context.Background(), "test"))
	require.Equal(t, len(schedulerCanonicalBuckets(0))+len(schedulerCanonicalBuckets(groupID))+3, cache.captureAttemptCount())
	require.Equal(t, 2*len(schedulerSnapshotPlatforms())+7, accounts.callCount())
	groups.mu.Lock()
	require.Equal(t, 1, groups.listCalls)
	groups.mu.Unlock()
...
func TestSchedulerFullRebuildActiveTombstoneFreshInactiveOrMissingFiltersAllGroupTasks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, len(schedulerSnapshotPlatforms())+2, accounts.groupCallCount(0))
			require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Equal(t, len(schedulerSnapshotPlatforms())+2, accounts.callCount())
	require.Empty(t, cache.tokens())
}
...
func TestSchedulerFullRebuildActiveTombstoneLazyRecoveryDiscardsPartialCaptureTasks(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.False(t, held)
		require.Equal(t, len(schedulerCanonicalBuckets(groupID)), reopenCount)
		require.Equal(t, len(schedulerCanonicalBuckets(groupID))+len(schedulerSnapshotPlatforms())+1, capturesAtFirstDB)
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Equal(t, len(schedulerSnapshotPlatforms())+5, accounts.callCount())
	require.Equal(t, len(schedulerSnapshotPlatforms())+5, accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	require.Equal(t, 2*len(schedulerSnapshotPlatforms())+4, accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, 2*(2*len(schedulerSnapshotPlatforms())+4), accounts.callCount())
