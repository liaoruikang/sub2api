...
//
//  1. (用户, 分组) limited_time_rpm_override — 仅在分组限时倍率窗口激活时生效。
//  2. (用户, 分组) rpm_override             — 最细粒度：管理员为特定用户在特定分组设定的专属限额。
//     override=0 表示该用户在该分组免检（绿灯），但 user 级全局上限仍然生效。
//  3. group.rpm_limit                        — 分组级：该分组的统一 RPM 容量（仅当无 override 时生效）。
//  4. user.rpm_limit                         — 用户级全局硬上限：无论 override/group 如何配置，始终生效。
//
...
func (s *BillingCacheService) checkRPM(ctx context.Context, user *User, group *Group) error {
	...
	// ── 第一层：分组级检查（limited-time override → limited-time group cap → regular override → regular group cap） ──
	if group != nil {
		groupLayerHandled := false
		if group.IsLimitedTimeMultiplierActiveAt(time.Now()) {
			var limitedTimeOverride *int
			if user.UserGroupLimitedTimeRPMOverride != nil {
				limitedTimeOverride = user.UserGroupLimitedTimeRPMOverride
			} else if s.userGroupRateRepo != nil {
				dbOverride, err := s.userGroupRateRepo.GetLimitedTimeRPMOverrideByUserAndGroup(ctx, user.ID, group.ID)
				if err != nil {
					logger.LegacyPrintf(
						"service.billing_cache",
						"Warning: limited-time rpm override lookup failed for user=%d group=%d: %v",
						user.ID, group.ID, err,
					)
				} else {
					limitedTimeOverride = dbOverride
				}
			}
			if limitedTimeOverride != nil {
				groupLayerHandled = true
				if *limitedTimeOverride > 0 {
					count, incErr := s.userRPMCache.IncrementUserGroupRPM(ctx, user.ID, group.ID)
					if incErr != nil {
						logger.LegacyPrintf(
							"service.billing_cache",
							"Warning: rpm increment (limited-time override) failed for user=%d group=%d: %v",
							user.ID, group.ID, incErr,
						)
						// fail-open
					} else if count > *limitedTimeOverride {
						return ErrGroupRPMExceeded
					}
				}
			} else if group.LimitedTimeRPMLimit > 0 {
				groupLayerHandled = true
				count, err := s.userRPMCache.IncrementUserGroupRPM(ctx, user.ID, group.ID)
				if err != nil {
					logger.LegacyPrintf(
						"service.billing_cache",
						"Warning: rpm increment (limited-time group) failed for user=%d group=%d: %v",
						user.ID, group.ID, err,
					)
					// fail-open
				} else if count > group.LimitedTimeRPMLimit {
					return ErrGroupRPMExceeded
				}
			}
		}

		if !groupLayerHandled {
			var override *int
