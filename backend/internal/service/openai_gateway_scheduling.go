...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, platform string, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability) *Account {
	...
	isSessionControlPreference := openAISessionControlPreferredAccountIDFromContext(ctx) == accountID
	...
	if !isSessionControlPreference && s.shouldPreferHighestSchedulingOverOpenAISticky(ctx, groupID, account, requestedModel, excludedIDs, requireCompact, requiredCapability) {
		return nil
	}
	...
	if isSessionControlPreference {
		_ = s.bindOpenAIStickySessionDuringSelection(ctx, groupID, sessionHash, accountID)
	} else {
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	...
	if preferredAccountID > 0 {
		stickyAccountID = preferredAccountID
	}
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		effectiveExcludedIDs := cloneExcludedAccountIDs(excludedIDs)
		var firstBusyControlledAccount *Account
		for {
			...
			if err != nil {
				if firstBusyControlledAccount != nil && (errors.Is(err, ErrNoAvailableAccounts) || errors.Is(err, ErrNoAvailableCompactAccounts)) {
					return s.newSelectionResult(ctx, firstBusyControlledAccount, false, nil, &AccountWaitPlan{
						AccountID:      firstBusyControlledAccount.ID,
						MaxConcurrency: firstBusyControlledAccount.Concurrency,
			...
			if acquireErr == nil && result != nil && !result.Acquired &&
				...
				if firstBusyControlledAccount == nil {
					firstBusyControlledAccount = account
				}
	...
	if sessionHash != "" {
		accountID := stickyAccountID
		isSessionControlPreference := preferredAccountID > 0 && preferredAccountID == accountID
		if accountID > 0 && !isExcluded(accountID) {
			...
			if err == nil {
				...
				if !clearSticky && isOpenAICompatibleAccountEligibleForRequest(ctx, account, platform, requestedModel, false, requiredCapability) {
					...
					} else if !isSessionControlPreference && s.shouldPreferHighestSchedulingOverOpenAISticky(ctx, groupID, account, requestedModel, excludedIDs, requireCompact, requiredCapability) {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
					} else {
	...
	tryAcquireFromLoadMap := func(loadMap map[int64]*AccountLoadInfo) (*AccountSelectionResult, bool, error) {
		...
		sortByHighestSchedulingLoadCandidates(available, false)
