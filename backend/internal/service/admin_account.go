...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	input.Extra = SanitizeAccountHighestSchedulingExtra(input.Extra)
	accountExtra, err := normalizeOpenAILongContextBillingExtra(input.Platform, input.Extra)
	if err != nil {
		return nil, err
	}
	...
	if _, err := s.ReconcileHighestSchedulingRotation(ctx, "account_create"); err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if _, updatesHighestScheduling := input.Extra[AccountExtraHighestSchedulingMode]; updatesHighestScheduling {
		managed, err := s.IsHighestSchedulingRotationManagedAccount(ctx, account)
		if err != nil {
			return nil, err
		}
		if managed {
			return nil, ErrHighestSchedulingRotationManaged()
		}
	}
...
func (s *adminServiceImpl) UpdateAccountExtra(ctx context.Context, id int64, updates map[string]any) error {
	updates = SanitizeAccountHighestSchedulingExtra(updates)
	delete(updates, UpstreamBillingProbeEnabledExtraKey)
	delete(updates, UpstreamBillingRateSyncEnabledExtraKey)
	delete(updates, UpstreamBillingProbeExtraKey)
	if _, updatesHighestScheduling := updates[AccountExtraHighestSchedulingMode]; updatesHighestScheduling && s.settingService != nil {
		account, err := s.accountRepo.GetByID(ctx, id)
		if err != nil {
			return err
		}
		...
		if managed {
			return ErrHighestSchedulingRotationManaged()
		}
	}
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	if protectHighestSchedulingUpdate {
