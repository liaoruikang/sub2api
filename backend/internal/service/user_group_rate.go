...
// RateMultiplier、RPMOverride 与 LimitedTimeRPMOverride 均为指针以支持"未设置"语义（NULL）。
type UserGroupRateEntry struct {
	...
	LimitedTimeRPMOverride *int     `json:"limited_time_rpm_override,omitempty"`
}
...
// GroupLimitedTimeRPMOverrideInput 批量设置分组限时倍率窗口 RPM override 的输入条目。
// LimitedTimeRPMOverride 为 *int 以支持清除（nil）语义。
type GroupLimitedTimeRPMOverrideInput struct {
	UserID                 int64 `json:"user_id"`
	LimitedTimeRPMOverride *int  `json:"limited_time_rpm_override"`
}
...
type UserGroupRateRepository interface {
	...
	GetLimitedTimeRPMOverrideByUserAndGroup(ctx context.Context, userID, groupID int64) (*int, error)
	...
	// SyncGroupLimitedTimeRPMOverrides 批量同步分组的限时倍率窗口用户专属 RPM（替换整组 limited_time_rpm_override 部分）。
	// 条目中 LimitedTimeRPMOverride 为 nil 时清空对应行的 limited_time_rpm_override；非 nil 时 upsert。
	SyncGroupLimitedTimeRPMOverrides(ctx context.Context, groupID int64, entries []GroupLimitedTimeRPMOverrideInput) error

	// ClearGroupLimitedTimeRPMOverrides 清空指定分组的所有 limited_time_rpm_override（整组限时 rpm 部分归 NULL）
