...
func TestOpenAIGatewayService_SelectAccountWithScheduler_HighestFullFallsBackToNormal(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	highest := Account{
		...
		Priority:    100,
		GroupIDs:    []int64{groupID},
		Extra:       map[string]any{AccountExtraHighestSchedulingMode: true},
	}
	normal := Account{
		ID:          37132,
		Platform:    PlatformOpenAI,
	...
	svc := &OpenAIGatewayService{
		accountRepo:      schedulerTestOpenAIAccountRepo{accounts: []Account{highest, normal}},
		cache:            &schedulerTestGatewayCache{},
		cfg:              cfg,
		rateLimitService: newOpenAIAdvancedSchedulerRateLimitService("true"),
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{
			loadMap: map[int64]*AccountLoadInfo{
				highest.ID: {AccountID: highest.ID, CurrentConcurrency: 0, LoadRate: 0},
				normal.ID:  {AccountID: normal.ID, CurrentConcurrency: 0, LoadRate: 0},
			},
			acquireResults: map[int64]bool{
				highest.ID: false,
				normal.ID:  true,
	...
	require.Equal(t, []int64{highest.ID, normal.ID}, acquiredIDs)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_HighestSchedulingOverridesNormalStickyBinding(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10105)
	accounts := []Account{
	...
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{"openai:session_hash_highest_sticky": 21501}}
	cfg := &config.Config{}
	cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled = false
	cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs = 15000
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.5
	concurrencyCache := schedulerTestConcurrencyCache{
		...
		loadMap: map[int64]*AccountLoadInfo{
			21501: {AccountID: 21501, LoadRate: 1, WaitingCount: 0},
			21502: {AccountID: 21502, LoadRate: 99, WaitingCount: 9},
		},
	}
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
	...
	selection, decision, err := svc.SelectAccountWithScheduler(ctx, &groupID, "", "session_hash_highest_sticky", "gpt-5.1", nil, OpenAIUpstreamTransportAny, false)
	require.NoError(t, err)
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_HighestSchedulingOverridesSubscriptionPriority(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10123)
	accounts := []Account{
		{
	...
	concurrencyCache := schedulerTestConcurrencyCache{
		acquireResults: map[int64]bool{21631: true, 21632: true},
		loadMap: map[int64]*AccountLoadInfo{
			21631: {AccountID: 21631, LoadRate: 0, WaitingCount: 0},
			21632: {AccountID: 21632, LoadRate: 90, WaitingCount: 1},
		},
	}
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
	...
	selection, decision, err := svc.SelectAccountWithScheduler(ctx, &groupID, "", "session_highest_over_subscription", "gpt-5.1", nil, OpenAIUpstreamTransportAny, false)
	require.NoError(t, err)
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
...
// exist.
func TestOpenAIGatewayService_SelectAccountWithScheduler_LoadBalanceTopKKeepsHighestSchedulingCandidates(t *testing.T) {
	ctx := context.Background()
	groupID := int64(109)
	accounts := []Account{
		{
	...
	concurrencyCache := schedulerTestConcurrencyCache{
		loadMap: map[int64]*AccountLoadInfo{
			36901: {AccountID: 36901, LoadRate: 90, WaitingCount: 4},
			36902: {AccountID: 36902, LoadRate: 5, WaitingCount: 0},
		},
	}
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(36901), selection.Account.ID)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
