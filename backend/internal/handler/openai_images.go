...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	groupUserReleaseFunc, groupAcquired := h.acquireResponsesGroupUserSlot(c, apiKey.Group, subject.UserID, parsed.Stream, &streamStarted, reqLog)
	if !groupAcquired {
		return
	}
	...
	for {
		...
		if summary := h.gatewayService.BuildOpenAIImageCostSummary(c.Request.Context(), usageInput); summary != nil {
			setUserImagePlaygroundSummary(c, summary)
		}
