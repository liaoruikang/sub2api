...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userTagRepository := repository.NewUserTagRepository(client)
	apiKeyCache := repository.NewAPIKeyCache(redisClient)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, userTagRepository, apiKeyCache, configConfig, billingCacheService, concurrencyService)
	...
	affiliateService := service.ProvideAffiliateService(affiliateRepository, settingService, apiKeyAuthCacheInvalidator, billingCacheService, secretEncryptor)
	...
	userService := service.ProvideUserService(userRepository, settingRepository, apiKeyAuthCacheInvalidator, billingCache, userTagRepository)
	...
	userImageHandler := handler.NewUserImageHandler(apiKeyService, subscriptionService)
	...
	highestSchedulingRotationReconciler := service.NewHighestSchedulingRotationReconciler(accountRepository, settingService)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, openAI403CounterCache, settingService, compositeTokenCacheInvalidator, highestSchedulingRotationReconciler)
	...
	announcementService := service.NewAnnouncementService(announcementRepository, announcementReadRepository, userRepository, userSubscriptionRepository, groupRepository, userTagRepository)
	...
	groupPriceMonitorService := service.ProvideGroupPriceMonitorService(settingRepository, groupRepository, announcementRepository, adminService)
	groupPriceMonitorHandler := admin.NewGroupPriceMonitorHandler(groupPriceMonitorService)
	...
	userTagService := service.NewUserTagService(userTagRepository, userRepository, groupRepository, apiKeyAuthCacheInvalidator)
	userTagHandler := admin.NewUserTagHandler(userTagService)
	...
	emailVerificationHandler := admin.NewEmailVerificationHandler(authService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, groupPriceMonitorHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, userTagHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, emailVerificationHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	grokVideoJobService := service.NewGrokVideoJobService(grokVideoJobRepository, openAIGatewayService)
	grokVideoJobHandler := handler.NewGrokVideoJobHandler(grokVideoJobService)
	seedanceRepository := repository.NewSeedanceRepository(db)
	seedanceService := service.NewSeedanceService(seedanceRepository, accountRepository, gatewayService, httpUpstream, tlsFingerprintProfileService, openAIGatewayService)
	seedanceHandler := handler.NewSeedanceHandler(seedanceService, billingCacheService, apiKeyService, concurrencyService, contentModerationService, coordinator)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, userImageHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, grokVideoJobHandler, seedanceHandler, grokVideoJobService, idempotencyCoordinator, idempotencyCleanupService)
