    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
                ...
                <div class="overflow-x-auto">
                  <table class="min-w-full text-sm">
                    ...
                    <tbody class="space-y-2">
                      <tr v-for="p in (['anthropic', 'openai', 'gemini', 'antigravity', 'grok', 'seedance'] as const)" :key="p" class="align-top">
                        <td class="pr-4 py-1">
                    ...
                    <div class="border-t border-gray-100 pt-4 dark:border-dark-700">
                      ...
                      <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                          ...
                          <tbody>
                            <tr v-for="p in (['anthropic', 'openai', 'gemini', 'antigravity', 'grok', 'seedance'] as const)" :key="`${authSource.source}-pq-${p}`" class="align-top">
                              <td class="pr-4 py-1">
                                <span class="font-mono text-xs text-gray-700 dark:text-gray-300">{{ p }}</span>
                              </td>
                              <td class="pr-4 py-1">
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.videoJobs.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.videoJobs.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.videoJobs.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.videoJobs.enabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.video_jobs_enabled" />
            </div>
          </div>
        </div>
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      video_jobs_enabled: form.video_jobs_enabled,
      // Affiliate (邀请返利) feature switch
