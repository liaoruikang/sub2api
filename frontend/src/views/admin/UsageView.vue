...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('admin.usage.account'), t('admin.usage.sessionId'), t('usage.requestedModel'), t('usage.sentUpstreamModel'), t('usage.upstreamResponseModel'), t('usage.upstreamModelMismatch'), t('usage.reasoningEffort'), t('admin.usage.group'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        log.created_at, log.user?.email || '', log.api_key?.name || '', log.account?.name || '', log.session_id || '', log.model,
