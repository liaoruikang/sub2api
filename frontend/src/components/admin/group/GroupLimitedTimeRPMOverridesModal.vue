<template>
  <BaseDialog :show="show" :title="t('admin.groups.limitedTimeRpmOverridesTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      <div class="flex flex-wrap items-center gap-3 rounded-lg bg-gray-50 px-4 py-2.5 text-sm dark:bg-dark-700">
        ...
        <span class="text-gray-400">|</span>
        <span class="text-gray-600 dark:text-gray-400">
          {{ t('admin.groups.limitedTimeGroupRpmDefault') }}: {{ group.limited_time_rpm_limit || 0 }}
        </span>
      </div>

      <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-600">
        <h4 class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
          {{ t('admin.groups.addUserLimitedTimeRpm') }}
        </h4>
        <p class="mb-3 text-xs text-gray-500 dark:text-gray-400">
          {{ t('admin.groups.limitedTimeRpmHint') }}
        </p>
      ...
      <div v-else>
        <h4 class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
          {{ t('admin.groups.limitedTimeRpmOverrides') }} ({{ localEntries.length }})
        </h4>

        <div v-if="localEntries.length === 0" class="py-6 text-center text-sm text-gray-400 dark:text-gray-500">
          {{ t('admin.groups.noLimitedTimeRpmOverrides') }}
        </div>
              ...
              <table class="w-full text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-gray-200 bg-gray-50 dark:border-dark-600 dark:bg-dark-700">
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400" :title="t('admin.groups.columns.limitedTimeRpmOverrideHint')">{{ t('admin.groups.columns.limitedTimeRpmOverride') }}</th>
                  ...
                  >
                    ...
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
                        ...
                        min="0"
                        autocomplete="off"
                        :value="entry.limited_time_rpm_override"
...
import type { GroupLimitedTimeRPMOverrideEntry } from '@/api/admin/groups'
...
interface LocalEntry extends GroupLimitedTimeRPMOverrideEntry {}
...
const serverEntries = ref<GroupLimitedTimeRPMOverrideEntry[]>([])
...
const isDirty = computed(() => {
  ...
  const serverMap = new Map(serverEntries.value.map(e => [e.user_id, e.limited_time_rpm_override]))
  return localEntries.value.some(e => serverMap.get(e.user_id) !== e.limited_time_rpm_override)
})
...
const cloneEntries = (entries: GroupLimitedTimeRPMOverrideEntry[]): LocalEntry[] => entries.map(e => ({ ...e }))

const loadEntries = async () => {
  if (!props.group) return
  loading.value = true
  try {
    serverEntries.value = await adminAPI.groups.getGroupLimitedTimeRPMOverrides(props.group.id)
    localEntries.value = cloneEntries(serverEntries.value)
    adjustPage()
  } catch (error) {
    appStore.showError(t('admin.groups.failedToLoad'))
    console.error('Error loading limited-time RPM overrides:', error)
  } finally {
    loading.value = false
  }
}
...
const handleAddLocal = () => {
  ...
  const entry: LocalEntry = {
    ...
    limited_time_rpm_override: newRpm.value
  }
  if (idx >= 0) {
...
const updateLocalRpm = (userId: number, value: string) => {
  ...
  if (entry) entry.limited_time_rpm_override = num
}
...
const clearAllLocal = async () => {
  ...
  clearing.value = true
  try {
    await adminAPI.groups.clearGroupLimitedTimeRPMOverrides(props.group.id)
    localEntries.value = []
    serverEntries.value = []
    appStore.showSuccess(t('admin.groups.limitedTimeRpmSaved'))
  } catch (error) {
    appStore.showError(t('admin.groups.failedToSave'))
    console.error('Error clearing limited-time RPM overrides:', error)
  } finally {
...
const handleSave = async () => {
  ...
  try {
    const entries = localEntries.value.map(e => ({
      user_id: e.user_id,
      limited_time_rpm_override: e.limited_time_rpm_override
    }))
    await adminAPI.groups.batchSetGroupLimitedTimeRPMOverrides(props.group.id, entries)
    appStore.showSuccess(t('admin.groups.limitedTimeRpmSaved'))
    emit('success')
    emit('close')
  } catch (error) {
    appStore.showError(t('admin.groups.failedToSave'))
    console.error('Error saving limited-time RPM overrides:', error)
  } finally {
    saving.value = false
  }
}
