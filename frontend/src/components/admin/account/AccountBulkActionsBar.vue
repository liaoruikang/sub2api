    ...
    <div class="flex gap-2">
      <template v-if="selectedIds.length > 0">
        ...
        <button
          ...
          :disabled="highestSchedulingManaged"
          :title="highestSchedulingManaged ? t('admin.accounts.highestSchedulingRotation.managedBulkHint') : undefined"
        >{{ t('admin.accounts.bulkActions.enableHighestScheduling') }}</button>
        <button
          @click="$emit('toggle-highest-scheduling', false)"
          class="btn btn-warning btn-sm"
          :disabled="highestSchedulingManaged"
          :title="highestSchedulingManaged ? t('admin.accounts.highestSchedulingRotation.managedBulkHint') : undefined"
        >{{ t('admin.accounts.bulkActions.disableHighestScheduling') }}</button>
...
defineProps<{
  ...
  highestSchedulingManaged?: boolean
}>()
defineEmits([
  'delete',
  'edit-selected',
