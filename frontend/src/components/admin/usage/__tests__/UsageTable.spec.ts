...
const messages: Record<string, string> = {
  ...
  'admin.usage.billingModeImage': 'Image',
	...
	'admin.usage.sessionIdCopied': 'Session ID copied',
	'keys.copied': 'Copied',
...
describe('admin UsageTable request ID column', () => {
  ...
  it('renders and copies the Session ID', async () => {
    const writeText = vi.fn().mockResolvedValue(undefined)
    vi.stubGlobal('navigator', { clipboard: { writeText } })

    const wrapper = mount(UsageTable, {
      props: {
        data: [{ ...baseImageRow, session_id: 'session-admin-visible-id' }],
        loading: false,
    ...
    expect(wrapper.text()).toContain('session-admin-visible-id')
    await wrapper.get('button[title="Copy to clipboard"]').trigger('click')

    expect(writeText).toHaveBeenCalledWith('session-admin-visible-id')
    expect(appStoreMocks.showSuccess).toHaveBeenCalledWith('Session ID copied')
  })
})
