...
const form = reactive({ email: '', password: '', username: '', notes: '', role: 'user', concurrency: 1, rpm_limit: 0, tag_ids: [] as number[], customAttributes: {} as UserAttributeValuesMap })

watch(() => props.user, (u) => {
  if (u) {
    Object.assign(form, { email: u.email, password: '', username: u.username || '', notes: u.notes || '', role: u.role || 'user', concurrency: u.concurrency, rpm_limit: u.rpm_limit ?? 0, tag_ids: (u.tags || []).map((tag) => tag.id), customAttributes: {} })
...
const handleUpdateUser = async () => {
  ...
  try {
    ...
    await adminAPI.tags.updateUserTags(userId, form.tag_ids)
