<template>
  <div class="flex flex-col gap-0.5">
    ...
    <!-- 会话数量限制 -->
    <CapacityBadge v-if="showSessionLimit" :color-class="sessionLimitClass" :tooltip="sessionLimitTooltip" :current="activeSessions" :max="sessionLimitMax">
      <svg class="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
const showSessionLimit = computed(
  () =>
    ...
    (props.account.platform === 'openai' &&
      ...
      props.account.openai_session_control_enabled === true &&
      (props.account.openai_session_max_count ?? 0) > 0)
)

const activeSessions = computed(() => props.account.active_sessions ?? 0)
const isOpenAISessionControl = computed(
  () =>
    props.account.platform === 'openai' &&
    props.account.type === 'oauth' &&
    props.account.openai_session_control_enabled === true
)
const sessionLimitMax = computed(() =>
  isOpenAISessionControl.value
    ? props.account.openai_session_max_count ?? 0
    : props.account.max_sessions ?? 0
)
...
const sessionLimitClass = computed(() => {
  ...
  const max = sessionLimitMax.value
  if (current >= max) return 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
...
const sessionLimitTooltip = computed(() => {
  ...
  const current = activeSessions.value
  const max = sessionLimitMax.value
  if (isOpenAISessionControl.value) {
    const timeout = formatSessionTimeout(props.account.openai_session_idle_timeout_seconds ?? 86400)
    if (current >= max) {
      const messageKey = props.account.openai_session_slot_rotation_enabled
        ? 'admin.accounts.capacity.sessions.openaiFullRotation'
        : 'admin.accounts.capacity.sessions.openaiFull'
      return t(messageKey, { timeout })
    }
    return t('admin.accounts.capacity.sessions.openaiNormal', { timeout })
  }
...
const formatSessionTimeout = (seconds: number) => {
  if (seconds % 86400 === 0) return `${seconds / 86400}${t('admin.accounts.openai.sessionControl.units.days')}`
  if (seconds % 60 === 0) return `${seconds / 60}${t('admin.accounts.openai.sessionControl.units.minutes')}`
  return `${seconds}${t('admin.accounts.openai.sessionControl.units.seconds')}`
}
