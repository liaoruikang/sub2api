...
// 生成唯一ID以避免多个对话框时ID冲突
const instanceId = createDialogId()
const dialogId = `modal-title-${instanceId}`

// 焦点管理
const dialogRef = ref<HTMLElement | null>(null)
const modalBodyRef = ref<HTMLElement | null>(null)
let previousActiveElement: HTMLElement | null = null
let dialogOpenTracked = false
...
watch(
  ...
  async (isOpen) => {
    if (isOpen) {
      ...
      if (!dialogOpenTracked) {
        dialogOpenTracked = true
        trackDialogOpen(instanceId)
      ...
      if (dialogOpenTracked) {
        dialogOpenTracked = false
        trackDialogClose(instanceId)
        if (!hasOpenDialogs()) document.body.classList.remove('modal-open')
      }
      // 恢复之前的焦点
