export default {
    accounts: {
      ...
      highestSchedulingModeHint: '仅提升已满足账号状态和可调度条件的候选账号。',
      enableHighestSchedulingAction: '启用最高调度',
      disableHighestSchedulingAction: '停用最高调度',
      highestSchedulingEnabled: '最高调度已启用',
      highestSchedulingDisabled: '最高调度已停用',
      failedToToggleHighestScheduling: '切换最高调度失败',
      bulkHighestSchedulingEnabled: '已为 {count} 个账号启用最高调度',
      bulkHighestSchedulingDisabled: '已为 {count} 个账号停用最高调度',
      bulkHighestSchedulingPartial: '最高调度部分更新成功：成功 {success} 个，失败 {failed} 个',
      bulkHighestSchedulingResultUnknown: '批量最高调度结果不完整，请稍后重试或刷新列表',
      highestSchedulingRotation: {
        shortTitle: '最高调度轮转',
        title: '最高调度轮转',
        description: '在指定分组和账号类型内自动维持指定数量的最高调度账号。',
        enabled: '启用轮转',
        enabledHint: '开启后会接管范围内最高调度开关，并按候选状态自动补位。',
        groups: '分组范围',
        groupsHint: '不选择具体分组时表示全部分组和未分组账号。',
        accountTypes: '账号类型',
        rotationCount: '轮转数量',
        activeCount: '当前最高调度账号',
        candidateCount: '符合条件账号',
        managedHint: '轮转启用后，范围内账号禁止手动开关最高调度；关闭轮转会清空范围内最高调度状态。',
        managedToggleHint: '该账号的最高调度由轮转托管，不能手动切换。',
        managedBulkHint: '选中账号包含轮转托管账号，不能批量切换最高调度。',
        reconcile: '立即同步',
        saved: '最高调度轮转配置已保存',
        reconciled: '最高调度轮转已同步',
        loadFailed: '加载最高调度轮转配置失败',
        saveFailed: '保存最高调度轮转配置失败',
        reconcileFailed: '同步最高调度轮转失败'
      },
      ...
      bulkActions: {
        ...
        enableHighestScheduling: '批量启用最高调度',
        disableHighestScheduling: '批量停用最高调度',
        resetStatus: '批量重置状态',
        refreshToken: '批量刷新令牌',
...
rateSyncWarning: '已开启上游倍率同步的账号不能批量手工修改倍率，请先在账号编辑页关闭同步。',
        ...
        poolModeNotice: '适用于 OAuth、API Key 和 Bedrock 账号；勾选后会批量开启或关闭池模式。',
        invalidPoolModeRetryStatusCodes: '同账号重试状态码必须是 100-599 的整数，可用英文逗号或空格分隔',
