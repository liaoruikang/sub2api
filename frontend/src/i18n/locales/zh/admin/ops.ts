export default {
    ops: {
      ...
      runtime: {
        ...
        metricThresholds: '健康指标阈值',
        metricThresholdsHint: '用于运维健康判断与诊断提示，修改后立即应用到后续评估。',
        slaMinPercent: 'SLA 最低值（%）',
        slaMinPercentHint: '低于该百分比时判定 SLA 异常。',
        ttftP99MaxMs: 'TTFT P99 最大值（毫秒）',
        ttftP99MaxMsHint: '高于该首 Token 延迟时判定响应异常。',
        requestErrorRateMaxPercent: '请求错误率上限（%）',
        requestErrorRateMaxPercentHint: '请求错误率超过该值时触发异常判断。',
        upstreamErrorRateMaxPercent: '上游错误率上限（%）',
        upstreamErrorRateMaxPercentHint: '上游错误率超过该值时触发异常判断。',
        silencing: {
          title: '告警静默（维护模式）',
          enabled: '启用静默',
